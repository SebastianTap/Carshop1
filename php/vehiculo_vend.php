...
class Vehiculo{
    ...
    public function llenarListaModelo($id_modelo){
        
        echo "--- ".$id_modelo;
        
        $registros=mysqli_query($this->conectarBD(),"SELECT * FROM modelos WHERE id='$this->SelMarca'")or die("Error en la consulta".mysqli_error($this->conectarBD()));
        //$registros=mysqli_query($this->conectarBD(),"select * from modelos where = ?")or die("Error en la consulta".mysqli_error($this->conectarBD()));
            while($reg=mysqli_fetch_array($registros)){
                $seleccionado = '';
                if($reg[0] == $id_modelo){
                    $seleccionado = 'selected';
    ...
    public function listarVehiculoCards(){
        ...
        while ($reg=mysqli_fetch_array($registro)){
          ...
          //  echo'       <p>Descripcion: '.$reg['descripcion'].'</p>';
            echo '        <form action="mas_Informacion.php">'; 
